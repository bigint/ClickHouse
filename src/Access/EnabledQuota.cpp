...
struct EnabledQuota::Impl
{
    ...
    static void
    used(const Intervals & intervals, QuotaType quota_type, QuotaValue value, std::chrono::system_clock::time_point current_time)
    {
    ...
    static void
    usedPerNormalizedHash(const Intervals & intervals, UInt64 normalized_query_hash, std::chrono::system_clock::time_point current_time)
    {
...
void EnabledQuota::used(const std::pair<QuotaType, QuotaValue> & usage1, bool check_exceeded) const
{
    ...
    for (const auto & quota : *loaded)
        Impl::used(*quota->intervals, usage1.first, usage1.second, current_time);
    if (check_exceeded)
        for (const auto & quota : *loaded)
            Impl::checkExceeded(getUserName(), *quota->intervals, usage1.first, current_time);
}
...
void EnabledQuota::used(const std::pair<QuotaType, QuotaValue> & usage1, const std::pair<QuotaType, QuotaValue> & usage2, bool check_exceeded) const
{
    ...
    {
        Impl::used(*quota->intervals, usage1.first, usage1.second, current_time);
        Impl::used(*quota->intervals, usage2.first, usage2.second, current_time);
    }
    if (check_exceeded)
        for (const auto & quota : *loaded)
        {
            Impl::checkExceeded(getUserName(), *quota->intervals, usage1.first, current_time);
            Impl::checkExceeded(getUserName(), *quota->intervals, usage2.first, current_time);
        }
}
...
void EnabledQuota::used(const std::pair<QuotaType, QuotaValue> & usage1, const std::pair<QuotaType, QuotaValue> & usage2, const std::pair<QuotaType, QuotaValue> & usage3, bool check_exceeded) const
{
    ...
    for (const auto & quota : *loaded)
    {
        Impl::used(*quota->intervals, usage1.first, usage1.second, current_time);
        Impl::used(*quota->intervals, usage2.first, usage2.second, current_time);
        Impl::used(*quota->intervals, usage3.first, usage3.second, current_time);
    }
    if (check_exceeded)
        for (const auto & quota : *loaded)
        {
            Impl::checkExceeded(getUserName(), *quota->intervals, usage1.first, current_time);
            Impl::checkExceeded(getUserName(), *quota->intervals, usage2.first, current_time);
            Impl::checkExceeded(getUserName(), *quota->intervals, usage3.first, current_time);
        }
}
...
void EnabledQuota::used(const std::vector<std::pair<QuotaType, QuotaValue>> & usages, bool check_exceeded) const
{
    ...
    for (const auto & quota : *loaded)
        for (const auto & usage : usages)
            Impl::used(*quota->intervals, usage.first, usage.second, current_time);
    if (check_exceeded)
        for (const auto & quota : *loaded)
            for (const auto & usage : usages)
                Impl::checkExceeded(getUserName(), *quota->intervals, usage.first, current_time);
}
...
void EnabledQuota::usedForQuery(UInt64 normalized_query_hash, QuotaType quota_type, QuotaValue value, bool check_exceeded) const
{
    ...
    for (const auto & quota : *loaded)
    {
        ...
        if (target)
        {
            Impl::used(*target, quota_type, value, current_time);
            targets.push_back(std::move(target));
        }
    }
    if (check_exceeded)
...
void EnabledQuota::usedForQuery(UInt64 normalized_query_hash, std::initializer_list<std::pair<QuotaType, QuotaValue>> usages, bool check_exceeded) const
{
    ...
    for (const auto & quota : *loaded)
    {
        ...
        if (!target)
            continue;
        for (const auto & usage : usages)
            Impl::used(*target, usage.first, usage.second, current_time);
        targets.push_back(std::move(target));
    }
    if (check_exceeded)
        for (const auto & target : targets)
